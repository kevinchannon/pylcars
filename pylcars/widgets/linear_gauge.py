...
from .split_pill import SplitPill, SplitPillStyles
...
class LinearGauge(Widgets, QtWidgets.QWidget):
    ...
    *mode* is ``"left"``, ``"right"``, or ``"dual"``.  Pass *left* / *right*
    ...
    _ELEM_GAP = 6       # px gap between tick area and indicator pill
    ...
    def __init__(
        ...
        *,
        ...
        # Pre-compute gauge vertical bounds (mirrors paintEvent logic so that
        # set_value can reposition the indicator pill without a repaint first).
        ...
        gtitle_fm = QtGui.QFontMetrics(QtGui.QFont(self.default_font))
        title_h  = title_fm.height() + 10
        bottom_h = gtitle_fm.height() + 10
        self._gauge_top    = title_h + 6
        self._gauge_bottom = rect.height() - bottom_h - 6

        # Indicator pills (one per active side)
        self._left_pill:  Optional[SplitPill] = None
        self._right_pill: Optional[SplitPill] = None
        if left  and mode in ("left",  "dual") and self._left_ax  is not None:
            self._left_pill  = self._make_pill("left",  left)
        if right and mode in ("right", "dual") and self._right_ax is not None:
            self._right_pill = self._make_pill("right", right)
    ...
    def set_value(self, side: str, value: float) -> None:
        ...
        pill   = self._left_pill  if side == "left"  else self._right_pill
        config = self.left_config if side == "left"  else self.right_config
        if pill is not None and config is not None:
            vmin, vmax = config.range
            if vmin <= value <= vmax:
                marker_y = self._val_to_y(value, vmin, vmax,
                                          self._gauge_top, self._gauge_bottom)
                mh = config.marker_size
                geo = pill.geometry()
                pill.setGeometry(geo.x(), marker_y - mh // 2, geo.width(), mh)
                pill.set_value(_fmt_value(value), value)
                pill.show()
            else:
                pill.hide()
        self.update()
    ...
    def _tick_clearance(self, config: GaugeConfig) -> int:
        """Width from the axis line to the far edge of the tick labels."""
        ...
        return self._TICK_LEN + max_lw + self._ELEM_GAP

    def _pill_width(self, config: GaugeConfig) -> int:
        """Pixel width of the SplitPill indicator for *config*."""
        family    = self.default_font.family()
        marker_h  = config.marker_size
        minor_slot = marker_h // 2   # triangle occupies the same slot as a D cap

        val_pt = _points_for_height(family, marker_h) - 1
        val_fm = QtGui.QFontMetrics(QtGui.QFont(family, max(4, val_pt)))
        digit_count = max(len(_fmt_value(config.range[0])),
                          len(_fmt_value(config.range[1])))
        digit_text_w = val_fm.horizontalAdvance("-" + "8" * digit_count)
        txpad = 4
        digit_col_w = digit_text_w + 2 * txpad
        ...
        badge_w  = unit_fm.horizontalAdvance(config.unit) + 10 + minor_slot

        return minor_slot + digit_col_w + badge_w

    def _side_content_width(self, config: GaugeConfig) -> int:
        return self._tick_clearance(config) + self._pill_width(config)
    ...
    # ------------------------------------------------------------------
    # Indicator pill factory
    # ------------------------------------------------------------------

    def _make_pill(self, side: str, config: GaugeConfig) -> SplitPill:
        """Create a hidden SplitPill indicator for *side*."""
        zones_sorted  = sorted(config.zones, key=lambda z: z.start)
        color_ranges  = [(z.start, z.colour) for z in zones_sorted]
        default_color = zones_sorted[0].colour if zones_sorted else self.color

        axis_x        = self._left_ax if side == "left" else self._right_ax
        tick_clear    = self._tick_clearance(config)
        pw            = self._pill_width(config)
        mh            = config.marker_size

        if side == "left":
            orientation = SplitPillStyles.Orientation.MAJOR_LEFT
            pill_x = axis_x - tick_clear - pw
        else:
            orientation = SplitPillStyles.Orientation.MAJOR_RIGHT
            pill_x = axis_x + tick_clear
        ...
        pill = SplitPill(
            self,
            QtCore.QRect(pill_x, 0, pw, mh),
            label=config.unit,
            digit_count=digit_count,
            pill_color=default_color,
            pill_text_color="#000000",
            text_color=self.color,
            color_ranges=color_ranges,
            minor_style=SplitPillStyles.MinorStyle.TRIANGLE,
            major_style=SplitPillStyles.MajorStyle.D,
    ...
    def _draw_gauge(
        ...
        p.drawLine(axis_x, top,    outer_x, top)
        p.drawLine(outer_x, top,   outer_x, top    + self._BRACKET_LEG)
        p.drawLine(axis_x, bottom, outer_x, bottom)
        p.drawLine(outer_x, bottom, outer_x, bottom - self._BRACKET_LEG)
